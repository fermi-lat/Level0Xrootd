...
if __name__ == '__main__':
    ...
    def merge( opts ):
        ...
        mergespans = []
        nevt = nmerged = norphaned = nskipped = nfiled_orphans = 0
        ofd_merge = None
        ofd_orph  = None
        ...
        while True:
            ...
            try:
                ...
                if ofd_merge:
                    _log.info( 'closing %s at %d with %d skipped %d merged in %s' % \
                               (ofd_merge.name, eidx.sequence, nskipped, nmerged, (merge0, merge1) ) )
                    mergespans.append( ( merge0, merge1 ) )
                    ofd_merge.close()
                    ofd_merge = None
                if ofd_orph:
                    _log.info('closing %s at %d with %d orphans in %s' % \
                              (ofd_orph.name, eidx.sequence, nfiled_orphans, (orph0, orph1) ) )
                    ofd_orph.close()
                    ofd_orph = None
                break
            ...
            # record orphaned events for possible later delivery
            if eidx.sequence < e0 or eidx.sequence > e1:
                # count the orphans
                norphaned += 1
                nskipped += 1

                # if this orphan isn't "related" to the previous orphan (i.e. from the
                # same EPU and datagram segment), then close any existing output file
                if ofd_orph and ( eidx.apid != orph_apid or (eidx.datagrams-orph_dgm > 1) ):
                    _log.info('closing %s at %d with %d orphans in %s' % \
                              (ofd_orph.name, eidx.sequence, nfiled_orphans, (orph0, orph1) ) )
                    ofd_orph.close()
                    ofd_orph = None
                ...
                if not ofd_orph:
                    orph0 = eidx.sequence
                    nfiled_orphans = 0
                    ofd_orph = open( os.path.join( opts.outdir, 'orphan-r%010d-e%020d-a%04d-d%06d.idx' % \
                                                   (startedAt, eidx.sequence, eidx.apid, eidx.datagrams) ), 'w' )
                    _log.info( 'opening %s', ofd_orph.name )

                # store the orphan event
                orph_apid = eidx.apid
                orph_dgm  = eidx.datagrams
                orph1 = eidx.sequence
                nfiled_orphans += 1
                print >> ofd_orph, str( eidx )
            ...
            # also close any open orphans file
